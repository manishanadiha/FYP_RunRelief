...
const ResourceScreen = ({ route }) => {
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      if (userType === 'public_user') {
        fetchRequestedListings(userID);
      } else if (userType === 'organization') {
        fetchOrgListings(userID);
      }
  ...
  const fetchRequestedListings = async (userID) => {
    try {
      let url = `http://192.168.1.105:5000/listings/requested/${userID}`;
      if (filter !== 'all') {
        url += `?status=${filter}`;
      }
  ...
  const fetchOrgListings = async (userID) => {
    try {
      let url = `http://192.168.1.105:5000/listings/org/${userID}`;
      if (filter !== 'all') {
        url += `?status=${filter}`;
      }
      const response = await fetch(url);
      ...
      <View style={styles.filterContainer}>
        ...
        {userType === 'public_user' && (
          <>
            ...
            >
              <Text style={filter === 'Accepted' ? styles.filterTextActive : styles.filterText}>Accepted</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={filter === 'Rejected' ? styles.filterButtonActive : styles.filterButton}
              onPress={() => setFilter('Rejected')}
            >
              <Text style={filter === 'Rejected' ? styles.filterTextActive : styles.filterText}>Rejected</Text>
            </TouchableOpacity>
          </>
        )}
      </View>
