...
const UnverifiedUsersScreen = () => {
  ...
  useEffect(() => {
    ...
    const fetchUsers = async () => {
      try {
        const response = await fetch('http://192.168.1.105:5000/unverified/public');
  ...
  const handleVerify = async (userId) => {
    try {
      const response = await fetch(`http://192.168.1.105:5000/verify/public/${userId}`, {
        method: 'PUT',
        headers: {
            ...
            return (
              <View style={styles.userContainer}>
                <View style={styles.userDetails}>
                  ...
                  <Text style={styles.userText}>Document: {user.document}</Text>
